...
func TestCompiler_VirtualMachine(t *testing.T) {
	...
	err := filepath.WalkDir("./testdata", func(path string, d fs.DirEntry, err error) error {
		...
		srcPath := filepath.Join(path, "src.hvsl")
		...
		t.Run(path, func(t *testing.T) {
			...
			require.NotPanics(t, func() {
				...
				hsvlCompiler := NewCompiler()
				hsvlProgram, err := hsvlCompiler.Compile(srcPath, bytes.NewBuffer(src))
				...
				hvilCompiler := hvil.NewCompiler()

				hvilProgram := hsvlProgram.GenerateHVILProgram()

				err = hvilCompiler.CompileAST(hvilProgram)
				require.NoError(t, err)

				asm, err := hvilProgram.GenerateVirtualMachineAssembly()
				require.NoError(t, err)
